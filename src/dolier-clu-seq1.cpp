...
/*
 * Sequential kmers clustering algorithm. It works with kmers spectra. dolier-fvector is not required.
 */
...
char* scmd;
void pusage(){
	std::cout<<"Sequential kmers clustering using kmers spectra.\n";
	...
	std::cout<<"\t<output_file> output file. Only selected centroids are printed.\n";
	std::cout<<"\t<minDist> min distance (with a centroid) to inclued a kmer in a cluster.\n";
	...
	std::cout<<"\toptional [--weights] use weighted spectra.\n";
	std::cout<<"\toptional [--log-weights] use log-weighted spectra.\n";
	std::cout<<"\toptional [--yupac] not yet inplemented.\n";
	std::cout<<"\toptional [-v] verbose.\n";
}
...
//	double *weights = new double[vlength];
	...
	seq1_algorithm algo(vectors, f_sequences.size(), nlength, weights, pars.minDist);
	//algo.run();
