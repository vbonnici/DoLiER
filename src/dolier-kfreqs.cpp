...
char* scmd;
void pusage(){
	std::cout<<"Output the global number of occurrences of each kmers inside input sequences.\n";
...
- frequency(w,S) = frequency(rc(w),S)				se we can calculate only one of them
*/
...
void run_from_list(mpars_t &pars){
	...
	while(it.next()){
		...
		start_t = start_time();
		f_dcount = sas.count(it_codes, D, k, m);
		...
		if(pars.opt_r){
			...
			//if(compare(it_codes_rev, it_codes, k) != 0){
				start_t = start_time();
				f_dcount += sas.count(it_codes_rev, D, k, m);
				search_time += end_time(start_t);
//			}
//			else{
//				f_dcount += 2;
...
public:
	...
	{
		sas = new SASearcher(f_ms.seq, f_ds.SA, f_ms.seq_length);
	}
	~kmerlist_thread_t(){
		delete sas;
	}
	...
	static void* run(void* argsptr){
		...
		for(usize_t r=st->ranges[idx][0]; r<st->ranges[idx][1]; r++){
			...
			f_dcount += st->sas->count(it_codes, D, k, st->m);
			if(st->reverse){
				reverseComplement(it_codes, it_codes_rev, k);
//				if(compare(it_codes_rev, it_codes, k) != 0){
					f_dcount += st->sas->count(it_codes_rev, D, k, st->m);
//				}
//				else{
//					f_dcount *= 2;
//				}
